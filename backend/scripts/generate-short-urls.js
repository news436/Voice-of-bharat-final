// Backend script to generate short URLs for all existing articles
// Run this from the backend directory: node scripts/generate-short-urls.js
...
async function createShortUrlForArticle(article) {
  try {
    ...
    if (existingShortUrl) {
      ...
      return { success: true, skipped: true, shortId: existingShortUrl.short_id };
    }
    ...
    return { success: true, shortId, shortUrl };
...
async function generateShortUrlsForAllArticles() {
  ...
  try {
    ...
    for (let i = 0; i < articles.length; i++) {
      ...
      const result = await createShortUrlForArticle(article);
    ...
    // Summary
    console.log('\n📊 Summary:');
    console.log(`✅ Successfully created: ${successCount} short URLs`);
